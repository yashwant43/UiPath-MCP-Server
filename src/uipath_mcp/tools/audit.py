...
def register(mcp: FastMCP) -> None:
    ...
    async def list_audit_logs(
        ...
        entity_type: Annotated[
            str | None,
            Field(description="Filter by entity type e.g. Job | Robot | Asset | Queue"),
        ] = None,
        action: Annotated[str | None, Field(description="Filter by action e.g. Create | Update | Delete")] = None,
        ...
        try:
            ...
            data = await st.client.get("AuditLogs", params=params.build())
    ...
    async def export_audit_logs(
        ...
        try:
            ...
            params: dict = {"$orderby": "CreationTime desc"}
            if parts:
                params["$filter"] = " and ".join(parts)

            all_logs = await st.client.collect_all(
                "AuditLogs", params=params, max_items=max_records
            )
            structured = [AuditLog.model_validate(l).model_dump() for l in all_logs]
            return json.dumps(
                {"exported_count": len(structured), "logs": structured}, default=str
            )
        except UiPathError as e:
            return json.dumps(e.to_dict())
